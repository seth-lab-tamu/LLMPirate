...
from config import DATASET_PATH, GNN4IP_SCRIPT, GNN4IP_YAML, GNN4IP_MODEL
sys.path.insert(0,"../")
...
#     command_str = 'read_lib /mnt/research/Rajendran_Jeyavijayan/Students/Gohil_Vasudev/Research/RL/AI_vs_AI/RL_for_breaking_all/synthesis/libs_for_GNN4IP/NangateOpenCellLibrary_typical.lib; '
...
#     command_str+= 'read_lib /mnt/research/Rajendran_Jeyavijayan/Students/Gohil_Vasudev/Research/RL/AI_vs_AI/RL_for_breaking_all/synthesis/libs_for_GNN4IP/NangateOpenCellLibrary_typical_reduced.lib; '
#     command_str+= "map; "
#     command_str+= 'write ' + resyn_circuit_path

#     return command_str

def evaluate_vlog_GNN4IP(GNN4IP_thresh,rank,timestamp):
...
# def func(lst):
#     [old_rank, variant, random_sample_num, det_tools, mapping_file, mapping_strategy] = lst
#     rank = str(old_rank*(num_random_samples) +  (random_sample_num)) + "_" + variant.split("/")[-1].split("-")[0]
...
#         abc_out = subprocess.run(['/mnt/research/Rajendran_Jeyavijayan/Students/Gohil_Vasudev/Research/RL/AI_vs_AI/RL_for_breaking_all/abc_my/abc/abc -q "' + ABC_command_str + '"'],shell=True , capture_output=True, text=True)#, stdout=subprocess.DEVNULL)
#         if abc_out.stderr != '':
#             print(abc_out)
...
#             [GNN4IP_similarity_score,MOSS_similarity_score,Sim_similarity_score,Jplag_similarity_score],[old_rank,random_sample_num]]

def newfunc(lst):
    [rank, design1, design2] = lst
    ...
    os.system("cp " + design1 + " ./tmp/files_for_GNN4IP/tmp_"+str(rank)+"_dataset_for_GNN4IP/current_circuit/orig/topModule.v")
    os.system("cp " + design2 + " ./tmp/files_for_GNN4IP/tmp_"+str(rank)+"_dataset_for_GNN4IP/current_circuit/pirated/topModule.v")

    timestamp = time.time()
    my_print("env_" + str(rank)+": Starting GNN4IP evaluation")
    GNN4IP_similarity_score = evaluate_vlog_GNN4IP(GNN4IP_thresh,rank,timestamp)
    if GNN4IP_similarity_score == None:
        # pass
        GNN4IP_successful = False
    elif GNN4IP_similarity_score<GNN4IP_thresh:
...
if __name__ == "__main__":
    ...
    # det_tools = ['GNN4IP']#,'MOSS','SIM','Jplag']#,'Sim','Jplag'] #'MOSS',

    designs = ['AES','GPS','MOR1KX']#['IBEX']# ['c432','c499','c880']

    variants = []
    for design in designs:
        variants.extend(glob.glob(os.path.join(DATASET_PATH, design.upper(), "*", "topModule.v")))

    
    args = []
    for design1 in variants:
        for design2 in variants:
            args.append([rank,design1,design2])
            rank+=1
    no_threads=len(args)
    with mp.Pool(processes=no_threads) as pool:
        results = list(pool.imap(newfunc, args, chunksize=1))
    
    cnt = 0
    for design1 in variants:
        for design2 in variants:
            if results[cnt][0] == True:
                my_print(f"{design1} vs. {design2}: similar: similarity score: {results[cnt][1]}")
            elif results[cnt][0] == False:
                my_print(f"{design1} vs. {design2}: NOT similar: similarity score: {results[cnt][1]}")
            else:
                my_print("ERROR! expected binary value")
            cnt+=1
    
    # all_successes = [result[0] for result in results]
    # all_similarity_scores = [result[1] for result in results]
    # all_identifiers = [result[2] for result in results]
    ...
    # master_results_file_name = "./master_results_slow_and_not_sure_for_case_study_against_all.pkl"
    master_results_file_name = "./master_results_slow_and_not_sure_for_case_study_against_all_AES.pkl"
    ...
    for mapping_file in mapping_files:
        ...
        for mapping_strategy in mapping_strategies:
            ...
            # for random_sample_num in range(1,num_random_samples+1):
            #     # print(variants[0])
            #     my_print("Random sample num:", random_sample_num)
            #     print()

            no_threads=5#len(variants)
            args = []
            for rank in range(len(variants)):
                for random_sample_num in range(num_random_samples):
                    args.append([rank,variants[rank],random_sample_num,det_tools,mapping_file,mapping_strategy])
            with mp.Pool(processes=no_threads) as pool:
            ...
            for random_sample_num in range(num_random_samples):
                my_print("Random sample num:", random_sample_num)
                successes = all_successes[random_sample_num::num_random_samples]
                similarity_scores = all_similarity_scores[random_sample_num::num_random_samples]
