...
def get_new_cached_circuit_mapping_for_ablation_study(mapping_file,mapping_files_to_LLMs,cached_circuit_mapping_original):
    with open("./saved_success_counts_dict_for_ablation.pkl",'rb') as f:
        success_counts = pickle.load(f)
    curr_LLM = mapping_files_to_LLMs[mapping_file]
    cached_circuit_mapping_new = {}
    for gate_type in cached_circuit_mapping_original:
        if gate_type not in cached_circuit_mapping_new:
            cached_circuit_mapping_new[gate_type] = {}
        for target_mapping_gate in cached_circuit_mapping_original[gate_type]:
            success_in_first_attempt = success_counts[curr_LLM][gate_type][target_mapping_gate][1]
            if success_in_first_attempt:
                cached_circuit_mapping_new[gate_type][target_mapping_gate] = copy.deepcopy(cached_circuit_mapping_original[gate_type][target_mapping_gate])
    return cached_circuit_mapping_new
...
def evaluate_vlog_GNN4IP(generic_version_of_current_circuit,GNN4IP_thresh,rank,timestamp):
    ...
    if 'error' in out.stderr.lower():
        ...
        if similarity_score<GNN4IP_thresh:
            #save_circ_name = "./mp_successful_circuits_ablation_study_wo_SolC/against_GNN4IP/"+self.generic_version_of_current_circuit.split("/")[-2]+"/env_"+str(self.rank)+"_"+str(time.time()).replace(".","_")+".v"
            os.makedirs("./successful_circuits_ablation_study_wo_SolC/against_GNN4IP/"+generic_version_of_current_circuit.split("/")[-2]+"/",exist_ok=True)
            save_circ_name = "./successful_circuits_ablation_study_wo_SolC/against_GNN4IP/"+generic_version_of_current_circuit.split("/")[-2]+"/env_"+str(rank)+"_"+str(timestamp).replace(".","_") + "_similarity_score_minus_"+str(-similarity_score)+ ".v"
            # self.f = open("./tmp_mp/files_for_GNN4IP/tmp_"+str(self.rank)+"/training_log.log",'a+')
...
def evaluate_vlog_MOSS(generic_version_of_current_circuit,MOSS_thresh,rank,timestamp,mapping_file,mapping_strategy,random_sample_num):
    ...
    os.makedirs("./saved_circuits_for_MOSS_ablation_study_wo_SolC/"+curr_LLM+"/"+curr_variant+"/",exist_ok=True)
    tmp_name_pirated = "./saved_circuits_for_MOSS_ablation_study_wo_SolC/"+curr_LLM+"/"+curr_variant+"/" + "pirated_mapping_strat_"+mapping_strategy+"_random_sample_num_"+str(random_sample_num)+".v"
    tmp_name_orig = "./saved_circuits_for_MOSS_ablation_study_wo_SolC/"+curr_LLM+"/"+curr_variant+"/" + "orig.v"
    ...
    try:
        ...
        if similarity_score<MOSS_thresh:
            os.makedirs("./successful_circuits_ablation_study_wo_SolC/against_MOSS/"+generic_version_of_current_circuit.split("/")[-2]+"/",exist_ok=True)
            save_circ_name = "./successful_circuits_ablation_study_wo_SolC/against_MOSS/"+generic_version_of_current_circuit.split("/")[-2]+"/env_"+str(rank)+"_"+str(timestamp).replace(".","_") + "_similarity_score_"+str(similarity_score)+ ".v"
            # subprocess.run(["cp", "./tmp/files_for_GNN4IP/tmp_"+str(rank)+"_dataset_for_GNN4IP/current_circuit/pirated/topModule.v"\
            # , save_circ_name], stdout=subprocess.DEVNULL)
        ...
        if (run_process.stderr):
            ...
            if "consists for" in out:
                ...
                if similarity_score<Sim_thresh:
                    os.makedirs("./successful_circuits_ablation_study_wo_SolC/against_Sim/"+generic_version_of_current_circuit.split("/")[-2]+"/",exist_ok=True)
                    save_circ_name = "./successful_circuits_ablation_study_wo_SolC/against_Sim/"+generic_version_of_current_circuit.split("/")[-2]+"/env_"+str(rank)+"_"+str(timestamp).replace(".","_") + "_similarity_score_"+str(similarity_score)+ ".v"
                    # subprocess.run(["cp", "./tmp/files_for_GNN4IP/tmp_"+str(rank)+"_dataset_for_GNN4IP/current_circuit/pirated/topModule.v"\
                    # , save_circ_name], stdout=subprocess.DEVNULL)
                    os.system("cp " + pirated_file_path + " " + save_circ_name)
                #     my_print("env_"+str(rank)+": MOSS: Found circuit that fools MOSS! " + save_circ_name + "\n")
        ...
        if (run_process.stderr):
            if "error" in run_process.stderr.lower():
                ...
                if similarity_score<Jplag_thresh:
                    os.makedirs("./successful_circuits_ablation_study_wo_SolC/against_Jplag/"+generic_version_of_current_circuit.split("/")[-2]+"/",exist_ok=True)
                    save_circ_name = "./successful_circuits_ablation_study_wo_SolC/against_Jplag/"+generic_version_of_current_circuit.split("/")[-2]+"/env_"+str(rank)+"_"+str(timestamp).replace(".","_") + "_similarity_score_"+str(similarity_score)+ ".v"
                    # subprocess.run(["cp", "./tmp/files_for_GNN4IP/tmp_"+str(rank)+"_dataset_for_GNN4IP/current_circuit/pirated/topModule.v"\
                    # , save_circ_name], stdout=subprocess.DEVNULL)
                    os.system("cp " + pirated_file_path + " " + save_circ_name)
                #     my_print("env_"+str(rank)+": MOSS: Found circuit that fools MOSS! " + save_circ_name + "\n")
...
if __name__ == "__main__":
    ...
    mapping_files_to_LLMs = {"./cached_circuit_mapping_CoPilot_precise.pkl":"CoPilot",\
                    # "./cached_circuit_mapping_GPT3dot5_old_prompt.pkl",\
                    "./cached_circuit_mapping_GPT3dot5.pkl":"GPT3.5",\
                    "./cached_circuit_mapping_GPT4.pkl":"GPT4",\
                    "./cached_circuit_mapping_Claude.pkl":"Claude",\
                    "./cached_circuit_mapping_Gemini.pkl":"Gemini",\
                    "./cached_circuit_mapping_llama3.pkl":"Llama3-8B",\
                    "./cached_circuit_mapping_codellama7b.pkl":"CL7B",\
                    "./cached_circuit_mapping_codellama13b.pkl":"CL13B",\
                        }
    ...
    master_results_file_name = "./master_results_ablation_study_wo_SolC.pkl"
    ...
    for mapping_file in mapping_files:
        with open(mapping_file,'rb') as f:
            cached_circuit_mapping_original = pickle.load(f)
        cached_circuit_mapping = get_new_cached_circuit_mapping_for_ablation_study(mapping_file,mapping_files_to_LLMs,cached_circuit_mapping_original)
        # print(cached_circuit_mapping)
        # exit()
