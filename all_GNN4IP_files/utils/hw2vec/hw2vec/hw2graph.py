...
class DataProcessor:
    ...
    def Vasudev_generate_pairs(self,const_folder):
        self.graph_pair_data = []
        for grph in self.graph_data:
            if grph.hw_name == const_folder:
                const_grph = grph
        #print("Constant graph: " + str())
        cnt=0
        for grph in self.graph_data:
            if grph.hw_name != const_folder:
                self.graph_pair_data.append((const_grph, grph))
                cnt+=1
        print("Generated " + str(cnt) + " pairs")
...
class DFGGenerator:
    ...
    def process(self, verilog_file, LLM_env_id):  # Vasudev
        dataflow_analyzer = VerilogDataflowAnalyzer(verilog_file, LLM_env_id, "top") # Vasudev
...
class CFGGenerator:
    ...
    def process(self, verilog_file):
        ...
        dataflow_analyzer = VerilogDataflowAnalyzer(self.verilog_file, "preprocess.output", "top") # Vasudev
        dataflow_analyzer.generate()
        binddict = dataflow_analyzer.getBinddict()
        terms = dataflow_analyzer.getTerms()
...
class HW2GRAPH:
    ...
    def code2graph(self, verilog_dir, LLM_env_id, profile=True): # Vasudev
        if profile:
            start = time()

        code_path = self.preprocess(verilog_dir)
        hw_graph = self.process(str(code_path), LLM_env_id) # Vasudev

        if profile:
            end = time()
            #print(str(code_path), ",", len(hw_graph.nodes), ",", len(hw_graph.edges), ",", end-start) ## Commented out for using with LLM
        return hw_graph
    ...
    # @profilegraph
    def process(self, hw_path, LLM_env_id): # Vasudev
        if self.cfg.graph_type == "CFG":
            ...
            nx_graph = generator.process(hw_path, LLM_env_id) # Vasudev
